...
class RouteAnimation:
    ...
    def detect_neighbor_axis_turns(self, path):
        ...
        for i in range(1, len(path)):
            ...
            if prev_axis is not None and curr_axis != prev_axis and i >= 2:
                ax, ay = path[i - 1][0] - path[i - 2][0], path[i - 1][1] - path[i - 2][1]
                bx, by = path[i][0] - path[i - 1][0], path[i][1] - path[i - 1][1]
    ...
    def spawn_forklift(self, ren, moves, current_idx, spawn_distance=1.0):
        from .engine_3d.objects.forklift.fork_addon import attach_fork_addon
        from .engine_3d.objects.forklift.forklift_make import araba_olustur_mecanum
        ...
        fork_assembly, _fork_parts = araba_olustur_mecanum()
        attach_fork_addon(
            fork_assembly,
            _fork_parts,
            fork_len=1.25,
        ...
        self.forklift_parts = dict(_fork_parts or {})
        self._fork_carriage = (self.forklift_parts.get("fork_addon", {}) or {}).get("carriage")
        levels = (self.forklift_parts.get("fork_addon", {}) or {}).get("levels")
        base = float(levels[0]) if isinstance(levels, (list, tuple)) and len(levels) >= 1 else 0.0
        n = len(levels) if isinstance(levels, (list, tuple)) else 4
        self._fork_levels = tuple(base + i * 1.0 for i in range(n))
    ...
    def talimat_hazirla(self):
        ...
        def _fmt_with_duration(instr, fork_level, phase):
            ...
            if tag == "dön":
                ...
                if pivot is not None:
                    text = f"{text}, {int(pivot[0])}, {int(pivot[1])}"
        ...
        for i, step_instrs in enumerate(all_steps, 1):
            ...
            first_fork_idx = next((idx for idx, ent in enumerate(approach_entries) if ent["tag"] in ("fork_yükselt", "fork_indir")), None)
            if first_fork_idx is not None:
                ...
                if approach_entries[first_fork_idx]["sec"] < route_total:
                    ...
                    approach_entries[first_fork_idx]["note"] = f" (toplamdan küçük diye 0 yazıldı, gerçek süre: {real_sec:.2f} sn)"
    ...
    def _on_anim_timer(self, obj, _ev):
        ...
        if self._active_cmd is None and self._parallel_fork_cmd is None and self._exec_idx >= len(self._exec_queue):
            self._stop_animation_timer()
            return
